...
- PROD (Singapore, ref byvbnkpiselvvulsvxgo) is applied ONLY via mcp__supabase__apply_migration (ask-gated). `supabase db push` to prod fails with SASL - do not attempt it. Local/dev may use the CLI from backend/.
