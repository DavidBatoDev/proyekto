...
1. **RLS (Supabase)**: every table created in the diff has policies in the same migration. Authorization must be server-side: profiles.is_consultant_verified for consultant surfaces, admin_profiles for platform administration, and the share_role ladder (owner > admin > editor > commenter > viewer) for projects. A client-only capability check is a finding. Self-referential policies on profiles/project_access/team-membership tables are recursion-prone - compare against the historical recursion-fix migrations.
