...
| 2026-05-03 | Slice 2 shipped: `project_shares` table + `ProjectAuthorizationService` + auto-grant rules + freelancer eligibility + project page UX cleanup | All 6 migrations applied to dev DB. 24 new backend tests pass (104 total). Web build green. PersonalWorkspaceService dual-writes legacy `project_members` + new `project_shares` so dependent-table RLS stays intact during the slice 2 → slice 3 transition window. |
| 2026-05-03 | Roadmap helper extension deferred to slice 3 | The original `get_user_roadmap_share_role(p_roadmap_id, p_user_id) RETURNS TEXT` (from `20260213000000_add_roadmap_sharing.sql`) cannot be `CREATE OR REPLACE`d to return `share_role` — Postgres rule. A `DROP CASCADE` would take out the RLS policies that currently reference it. Slice 3 introduces a new helper with a distinct name (`get_user_roadmap_effective_role`), migrates RLS policies in lockstep, then drops the old function. **Implication for slice 2**: project owners/admins do NOT yet auto-inherit roadmap edit rights via `project_shares` — they need an explicit `roadmap_shares` row to edit roadmaps until slice 3. Project-level access still works (covered by the slice 2 RLS swap on `projects`). Roadmap-only public links continue to work unchanged. |
| 2026-05-03 | Slice 3 split: ship Phase A now, defer `project_members` table drop to Slice 3b | Slice 3 originally bundled (a) marketplace gate hardening, (b) consultant-verification CHECK constraint, (c) roadmap helper, AND (d) drop `project_members` table. Phase A (a-c) ships cleanly: 2 migrations + ConsultantOnlyGuard. The table drop turns out to be much larger — discovered 14+ `project_members` reads across 6 modules (chat, project-time, admin, roadmaps, uploads, marketplace) plus a FK from `project_member_time_rates.project_member_id`. Each read site is a straightforward migration to `project_shares EXISTS` but cumulative; doing it all in one PR risks regressions across unrelated module surfaces. Split into Slice 3b with a per-module breakdown. |
| 2026-05-03 | Roadmap effective-role helper added as ADDITIVE function (slice 3 part) | `get_user_roadmap_effective_role(uid, roadmap_id) RETURNS share_role` consults `project_shares` as fallback. The legacy `get_user_roadmap_share_role(p_roadmap_id, p_user_id) RETURNS TEXT` is left untouched — it's still referenced by RLS policies. Migrating those policies to call the new function in lockstep is its own slice (3.10b). For now the new function is opt-in for backend service code; existing roadmap RLS keeps working unchanged. |
