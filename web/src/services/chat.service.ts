...
/** Lean preview of the message a reply quotes (content blank if deleted). */
...
export interface ChatMessage {
	...
	/** Set once the sender edits the message → drives the "(edited)" label. */
	edited_at?: string | null;
	/** Set when soft-deleted → the bubble renders a "message deleted" tombstone. */
	deleted_at?: string | null;
	/** Id of the message this one replies to (quote), if any. */
	reply_to_id?: string | null;
	/** Hydrated preview of the quoted message (server-attached on read). */
...
class ChatService {
	...
	listMembers(projectId: string): Promise<ChatMemberCandidate[]> {
		return this.request<ChatMemberCandidate[]>(
			`/projects/${projectId}/chat/members`,
			{
				method: "GET",
			},
		);
	...
	toggleReaction(messageId: string, emoji: string): Promise<{ ok: boolean }> {
		return this.request<{ ok: boolean }>(
	...
	toggleRoomStar(roomId: string): Promise<{ starred: boolean }> {
		return this.request<{ starred: boolean }>(`/chat/rooms/${roomId}/star`, {
			method: "POST",
		});
	}

	/** Edit a message's text (+ mentions). Server stamps edited_at. */
	editMessage(
		messageId: string,
		payload: { content: string; mentions?: ChatMention[] },
	...
	/** Soft-delete (unsend) a message. Server tombstones it; the row remains. */
