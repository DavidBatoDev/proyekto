			...
			if (payload.is_private && payload.memberIds?.length) {
				await Promise.all(
					payload.memberIds.map((id) =>
						chatService
							.addChannelMember(projectId, room.id, id)
							.catch(() => null),
					),
				);
			}
...
export function useDeleteChatMessageMutation() {
	...
	return useMutation({
		...
		// Soft delete: tombstone the message in place (set deleted_at + clear
		// content/attachments/mentions/reactions) instead of removing it, matching
		// what the server now returns on refetch.
		onMutate: async (payload) => {
			const key = chatKeys.roomMessages(payload.roomId);
			await queryClient.cancelQueries({ queryKey: key });
			const previous =
...
/**
 * Edit a message's text (+ mentions). Optimistically patches the content,
 * mentions, and `edited_at` of the matching message across the infinite-query
 * pages; rolls back on error and refetches on settle.
 */
export function useEditChatMessageMutation() {
	...
	return useMutation({
		mutationFn: (payload: {
			...
			chatService.editMessage(payload.messageId, {
...
export function useMarkRoomReadMutation(
	options: { projectId?: string; isDm?: boolean; currentUserId?: string } = {},
) {
	const queryClient = useQueryClient();
...
export function flattenRoomMessages(
	data:
		| {
...
export function findRoomByCounterpart(
	...
	for (const room of rooms) {
		...
		const hasUser = room.participants.some(
