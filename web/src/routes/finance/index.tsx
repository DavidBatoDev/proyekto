...
function FinancePage() {
	...
	const createContractMutation = useMutation({
		mutationFn: (projectId: string) => contractService.create(projectId, {}),
		onSuccess: (created) => {
		...
		<DashboardShell>
			<div className="app-shell-bg min-h-full px-5 py-4 md:px-8 md:py-5">
				...
				<CreateContractDialog
					...
					onCreate={(projectId) => createContractMutation.mutate(projectId)}
				/>
			</div>
		</DashboardShell>
	);
		...
		<ModalPortal>
			<div className="fixed inset-0 z-80 flex items-center justify-center p-4">
				...
				>
					<div className="flex items-start justify-between gap-4">
						<div>
							...
							<p className="mt-1 text-xs leading-5 text-muted-foreground">
								Choose the project this client agreement belongs to.
							</p>
						</div>
						<button
							type="button"
					...
					<div className="mt-3 max-h-64 overflow-y-auto rounded-lg border border-border p-1">
						{loading ? (
