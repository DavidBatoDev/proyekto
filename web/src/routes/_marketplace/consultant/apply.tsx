	...
	return (
		<div className="space-y-4">
			<p className="text-sm text-slate-600">
				Upload a government-issued photo ID to begin the verification process.
				Your documents are stored securely and only accessible to authorized
				admin staff.
			</p>
			...
			{docs.length > 0 && (
				<div className="space-y-3">
					{docs.map((doc) => (
						<div
							key={doc.id}
							className="flex items-center gap-4 p-4 bg-white border border-gray-200 rounded-xl group"
						>
							<div
							...
							<div className="flex-1 min-w-0">
								...
								<p className="text-xs text-gray-400">
									...
									Uploaded {fmtDate(doc.uploaded_at ?? doc.created_at)}
								</p>
							</div>
							<button
			...
			<EmptySlate
				message="Add identity document"
				onAdd={() => setIdDocModalOpen(true)}
			/>
			...
			{docs.length === 0 && (
				<div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700">
					<strong>Tip:</strong> Adding at least one verified identity document
					significantly improves your application approval rate.
				</div>
			)}
		</div>
	);
...
function TileOption({
	...
	return (
		<label
			className={`flex items-start gap-3 px-4 py-3 rounded-xl border cursor-pointer transition-all ${checked ? "border-slate-900 bg-slate-50 shadow-sm" : "border-slate-200 bg-white hover:border-slate-300"}`}
		>
			<input
				type="radio"
				name={name}
...
function Step2ExperienceNiche({
	formData,
	updateFormData,
}: {
	formData: FormData2;
	updateFormData: (u: Partial<FormData2>) => void;
}) {
	return (
		<div className="space-y-6">
				...
				<div className="grid grid-cols-2 gap-3">
					{[
						...
						<TileOption
							...
							value={opt.value}
							label={opt.label}
							checked={formData.years_of_experience === opt.value}
							onChange={() =>
								updateFormData({ years_of_experience: opt.value })
							}
						/>
					))}
				</div>
			...
			<div>
				<label className="block text-sm font-semibold text-slate-900 mb-2">
					Primary Industry / Niche <span className="text-red-500">*</span>
				</label>
				<div className="grid grid-cols-3 gap-2">
					{NICHES.map((n) => (
						<TileOption
							...
							value={n.value}
							label={n.label}
							checked={formData.primary_niche === n.value}
							onChange={() =>
								updateFormData({ primary_niche: n.value, custom_niche: "" })
							}
						/>
					))}
				</div>
						...
						{profile.languages.map((lang) => (
							...
							>
								<div className="flex-1">
									<p className="text-sm font-semibold text-gray-900">
										{lang.language?.name ?? lang.language_id}
									</p>
									<p className="text-xs text-gray-400 capitalize">
										{lang.fluency_level?.replace("_", " ")}
									</p>
								</div>
								<div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all shrink-0">
									<button
										...
										className="p-1 rounded text-gray-300 hover:text-slate-900 hover:bg-slate-100 transition-colors"
									>
					...
					<div className="relative">
						...
						<div className="space-y-0">
							{profile.experiences.map((exp) => (
								<div
									key={exp.id}
									className="relative flex items-start gap-4 pl-10 pb-6 last:pb-0 group"
								>
				...
				{profile.educations.length > 0 ? (
					<div className="space-y-3">
						{profile.educations.map((edu) => (
							<div
								key={edu.id}
								className="flex items-start gap-4 p-4 bg-white border border-gray-200 rounded-xl group"
							>
		...
		<div className="space-y-6">
			<div>
				...
				<p className="text-xs text-slate-600 mb-3">
					Tell us about your professional background and why you'd be a great
					consultant on this platform. Mention specific outcomes, industries, or
					methodologies you specialise in.
				</p>
				<textarea
					rows={11}
			...
			<div>
				<label className="block text-sm font-semibold text-slate-900 mb-2">
					Why do you want to join as a Consultant?
				</label>
				<textarea
					rows={4}
		...
		<div className="space-y-6">
			...
			<div className="border-t border-gray-100 pt-5">
				...
				<div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-0">
					...
					<ReviewRow
						label="Languages"
						value={plural(profile.languages.length, "language")}
					/>
					<ReviewRow
						label="Work Experience"
						value={plural(profile.experiences.length, "entry", "entries")}
					/>
					<ReviewRow
						label="Education"
						value={plural(profile.educations.length, "entry", "entries")}
					/>
					<ReviewRow
						label="Licenses"
						value={plural(profile.licenses.length, "license")}
					/>
					<ReviewRow
						label="Cover Letter"
						value={
				...
				<p className="text-xs text-slate-600 mt-3">
					Our team will manually review your profile within{" "}
					<strong>3–5 business days</strong>. Applications can only be submitted
					once — ensure all information is accurate.
				</p>
			</div>
		</div>
	);
...
function ConsultantApplyPage() {
	...
	const updateFormData = (updates: Partial<FormData2>) =>
		...
		setFormData((prev) => ({
			...
			years_of_experience:
				...
				existingApp.years_of_experience !== undefined
					...
					: "",
			primary_niche: draftNiche ? (isKnownNiche ? draftNiche : "other") : "",
	...
	const submitApp = useMutation({
		mutationFn: async () => {
			await applicationService.saveDraft({
				...
				primary_niche:
					formData.primary_niche === "other" && formData.custom_niche
						...
						>
							...
							>
								...
								<p className="text-sm text-gray-400 mb-8">
									Our admin team will review your profile and supporting
									evidence. You'll be notified within{" "}
