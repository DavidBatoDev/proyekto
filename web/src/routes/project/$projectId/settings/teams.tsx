...
function ProjectTeamsTab() {
  ...
  const callerIsOwner = isCallerOwner(allMembers, currentUserId);
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-slate-600">
            Attach a team to bring its members into this project. Pick which
            members participate — others on the team won't see this project.
          </p>
        </div>
        {canManageTeams && (
          <button
        ...
        <div>
          {curatedQuery.isLoading ? (
            ...
            <ul className="divide-y divide-slate-200">
              {curated.map((m) => {
                const targetAccess = allMembers.find(
                  (x) => x.user_id === m.user_id,
                );
                const outranked =
                  !callerIsOwner &&
                  targetAccess !== undefined &&
                  memberHasGate(targetAccess, "members.manage");
                return (
                  <CuratedMemberRow
                    key={m.user_id}
                    projectId={projectId}
                    ...
                    member={m}
                    canManage={canManageMembers && !outranked}
                  />
                );
              })}
            </ul>
...
function CuratedMemberRow({
  ...
  return (
    <li className="flex items-center justify-between px-5 py-3">
      <MemberDisplay user={member.user} fallbackId={member.user_id} />
      {canManage && !isSelf && (
        <button
          type="button"
...
const ROLE_OPTIONS: ProjectTeamDefaultRole[] = ["admin", "editor", "viewer"];
            ...
            {availableQuery.isLoading ? (
              ...
              <ul className="divide-y divide-slate-200">
                {available.map((m) => {
                  ...
                  const role =
                    lockedRole ?? pickedRoleByUserId[m.user_id] ?? "editor";
                  return (
                    <li
                      key={m.user_id}
                    ...
                    >
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        ...
                        >
                          {lockedRole && !ROLE_OPTIONS.includes(lockedRole) && (
                            <option value={lockedRole}>{lockedRole}</option>
                          )}
                          {ROLE_OPTIONS.map((r) => (
                        ...
                        <button
                          ...
                          onClick={() =>
                            addMutation.mutate({
                              userId: m.user_id,
                              role: lockedRole ? undefined : role,
                            })
                          }
...
function AttachTeamModal({
  ...
  const [pickedRoleByUserId, setPickedRoleByUserId] = useState<
    Record<string, ProjectTeamDefaultRole>
  >({});
  const [bulkRole, setBulkRole] = useState<ProjectTeamDefaultRole>("editor");
  ...
  const attachMutation = useMutation({
    mutationFn: () => {
      ...
      const members = teamMembers
        ...
        .map((m) =>
          existingRoleByUserId.has(m.user_id)
            ? { user_id: m.user_id }
            : {
                user_id: m.user_id,
                role: pickedRoleByUserId[m.user_id] ?? "editor",
              },
        );
    ...
    <ModalPortal>
      ...
      >
        <div
          ...
          onClick={(e) => e.stopPropagation()}
        >
          <h2 className="text-lg font-semibold text-slate-900">Attach team</h2>
          <p className="mt-1 text-sm text-slate-600">
                ...
                <div className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                  ...
                  <div className="flex items-center gap-2">
                    <select
                      value={bulkRole}
                      onChange={(e) =>
                        setBulkRole(e.target.value as ProjectTeamDefaultRole)
                      }
                  ...
                  {membersQuery.isLoading ? (
                    ...
                    <ul className="divide-y divide-slate-200">
                      {(membersQuery.data ?? []).map((m) => {
                        ...
                        const role =
                          existing ?? pickedRoleByUserId[m.user_id] ?? "editor";
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              {existing && !ROLE_OPTIONS.includes(existing) && (
