...
function Avatar({
	...
	if (url)
		return (
			<img src={url} className={`${cls} rounded-full object-cover shrink-0`} />
		);
	return (
		<div
			className={`${cls} rounded-full bg-amber-100 text-amber-700 font-bold flex items-center justify-center shrink-0`}
		>
			{initials(name)}
		</div>
	);
					...
					<div className="flex items-center gap-1.5 mt-1.5">
						<div className="w-5 h-5 rounded-full bg-gray-100 flex items-center justify-center shrink-0">
							<span className="text-[10px] font-bold text-gray-500">
								{initials(
									project.owner?.display_name ?? project.owner?.email ?? "O",
								)}
							</span>
						</div>
	...
	return (
		<div
			className={`group bg-white border rounded-xl p-4 transition-all cursor-pointer ${
				isSelected
					? "border-amber-400 shadow-sm bg-amber-50/20"
			...
			<div className="flex items-start gap-3">
				...
				<div className="flex-1 min-w-0">
					...
					<div className="flex items-center gap-2 mt-1.5 flex-wrap">
						{rate?.availability && (
							<AvailabilityBadge status={rate.availability} />
						)}
						{rate?.hourly_rate && (
				...
				<ChevronRight
					className={`w-4 h-4 shrink-0 mt-1 transition-colors ${isSelected ? "text-amber-500" : "text-gray-300"}`}
				/>
			</div>
...
function ConsultantProfilePanel({
	id,
	onClose,
}: {
	id: string;
				...
				{isLoading ? (
					...
					<div className="p-6 space-y-6">
						...
						<div className="relative rounded-xl overflow-hidden border border-gray-200">
							{/* Banner */}
							<div
								className={`h-20 w-full ${profile.banner_url ? "" : "bg-linear-to-br from-amber-100 to-amber-200"}`}
							>
								{profile.banner_url && (
									<img
							...
							<div className="grid grid-cols-2 gap-3">
								{profile.rate_settings.hourly_rate && (
									<div className="bg-gray-50 border border-gray-100 rounded-xl p-3">
										...
										<p className="text-sm font-bold text-gray-900 mt-0.5">
											${profile.rate_settings.hourly_rate}{" "}
											{profile.rate_settings.currency ?? "USD"}
										</p>
									</div>
								)}
								{profile.rate_settings.weekly_hours && (
									<div className="bg-gray-50 border border-gray-100 rounded-xl p-3">
										...
										<p className="text-sm font-bold text-gray-900 mt-0.5">
											{profile.rate_settings.weekly_hours} hrs/wk
										</p>
									</div>
								)}
							</div>
							...
							<section>
								...
								<div className="flex flex-wrap gap-2">
									{profile.languages.map((l) => (
										<span
											key={l.id}
											className="text-xs bg-blue-50 text-blue-700 border border-blue-100 px-2.5 py-1 rounded-full capitalize"
										>
											{(l as any).language?.name} ·{" "}
											{l.fluency_level?.replace(/_/g, " ")}
										</span>
									))}
								</div>
							</section>
							...
							<section>
								...
								<div className="flex flex-wrap gap-2">
									{profile.certifications.map((c) => (
										<span
											key={c.id}
											className="text-xs bg-purple-50 text-purple-700 border border-purple-100 px-2.5 py-1 rounded-full capitalize"
										>
											{c.name}
										</span>
									))}
								...
								<div className="grid grid-cols-1 gap-2">
									{profile.portfolios.map((p: any) => (
										...
										>
											...
											<div className="min-w-0 flex-1">
												<p className="text-sm font-semibold text-gray-800 truncate group-hover:text-amber-600">
													{p.title}
												</p>
												{p.description && (
...
function MatchPage() {
	...
	const { data: candidates = [], isLoading: candLoading } = useQuery({
		...
		queryFn: () =>
			adminService.searchConsultants({
	...
	const assignMutation = useMutation({
		...
		onSuccess: (_, vars) => {
			setAssignedMap((prev) => ({
				...prev,
				[vars.consultantId]: vars.projectId,
			}));
						...
						<div className="shrink-0 px-5 py-3 bg-white border-b border-gray-100 space-y-2">
							{selectedProject && (
								<div className="flex items-center gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-2">
									...
									<span>
										Showing consultants ranked for{" "}
											...
											>
												{NICHES.map((n) => (
													<option key={n} value={n}>
														{n === "all" ? "All niches" : n.replace(/_/g, " ")}
													</option>
												))}
											</select>
											<select
								...
								sorted.map((c) => (
									<ConsultantCard
										...
										isAssigning={
											...
											assignMutation.variables?.consultantId === c.id
										}
									/>
								))
							)}
