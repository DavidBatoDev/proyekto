...
function ApplicantAvatar({
	...
	return (
		<div
			className={`${dim} rounded-full bg-amber-100 flex items-center justify-center text-amber-700 font-bold shrink-0`}
		>
			{initials(name)}
		</div>
	);
...
function ApplicationDetailPanel({
	...
	const reject = useMutation({
		mutationFn: () =>
			adminService.rejectApplication(id, rejectReason || undefined),
		onSuccess: () => {
			qc.invalidateQueries({ queryKey: ["adminApps"] });
			qc.invalidateQueries({ queryKey: ["adminApp", id] });
						...
						{v && (
							<>
								...
								<VettingSection
									title="Languages"
									icon={Globe}
									count={v.languages.length}
								>
									{v.languages.map((lang) => (
										<VettingChip
											key={lang.id}
											label={`${lang.language?.name} — ${lang.fluency_level?.replace("_", " ")}`}
										/>
									))}
								</VettingSection>
								...
								<VettingSection
									...
									icon={Building2}
									count={v.experiences.length}
								>
									{v.experiences.map((exp) => (
										<div
											key={exp.id}
											className="text-xs text-gray-700 py-2 border-b border-gray-100 last:border-0"
										>
	...
	return (
		<div className="flex gap-3">
			{[
				...
				<div
					key={s.label}
					className={`border rounded-xl px-4 py-2.5 text-center min-w-[80px] ${s.bg}`}
				>
