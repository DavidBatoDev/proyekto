...
const moveFeatureAcrossEpics = (
	...
	return withoutFeature.map((epic) => {
		...
		const existingFeatures = (epic.features || []).filter(
			(f) => f.id !== featureId,
		);
		const reordered = orderedTargetFeatureIds
			.map((id) =>
				id === featureId
					? updatedFeature
					: existingFeatures.find((f) => f.id === id),
			)
...
export const useRoadmapStore = create<RoadmapStore>((set, get) => ({
			...
			for (const item of sortedCreated) {
				...
				if (item.node_type === "milestone") {
					...
					const newMilestone: RoadmapMilestone = {
						...
						status: ((op?.data?.status as string | undefined) ??
							...
							features: (e.features ?? []).map((f) => ({
								...
								tasks:
									f.id === targetFeatureId
										? [...(f.tasks ?? []).filter((t) => t.id !== task.id), task]
	...
	moveFeatureBetweenEpics: async (
		...
		try {
			...
			await featureService.update(featureId, {
				epic_id: targetEpicId,
				...
				const nextEpics = state.epics.map((epic) => ({
					...
					features: (epic.features || []).map((feature) => {
						...
						const optimisticTask: RoadmapTask = {
							...
							position: optimisticPosition,
							// Placeholder pending the server response, which computes the
							// real append-at-end value scoped to the destination column.
							board_order: 0,
	...
	updateTask: async (task: RoadmapTask) => {
		...
		try {
			...
			const storedDueDate = (currentTask as { due_date?: string | null })
				.due_date;
	...
	reorderTasksInColumn: async (
		...
		try {
			const reorderPatch = orderedTaskIds.map((taskId, index) => ({
				task_id: taskId,
				new_order_index: index,
			}));
			await taskService.reorderByStatus(roadmapId, status, reorderPatch);
		} catch (error) {
			console.error(
				`Failed to reorder tasks in column ${status} for roadmap ${roadmapId}:`,
				error,
			);
			set({ epics });
			throw error;
	...
	moveTaskToColumn: async (
		...
		const taskBeforeMove = findTaskById(epics, taskId);
		if (!taskBeforeMove || taskBeforeMove.status === targetStatus) return;

		const rollbackEpics = epics;
		const orderById = new Map(
			orderedTargetTaskIds.map((id, index) => [id, index]),
		);
		...
		try {
			// Step 1: move the task to the target column with a safe temp
			// board_order that won't collide with any existing task there.
			const safeBoardOrder = orderedTargetTaskIds.length * 1000 + 5000;
			await taskService.update(taskId, {
				status: targetStatus,
				board_order: safeBoardOrder,
			});

			// Step 2: reorder all tasks in the target column so every
			// board_order is correct.
