...
const baseTimeline = (
	overrides: Partial<AiActivityTimeline> = {},
): AiActivityTimeline => ({
	traceId: "trace-1",
...
describe("parallel step grouping", () => {
	const step = (
		seq: number,
		overrides: Partial<AiActivityStep> = {},
	): AiActivityStep => ({
		seq,
...
describe("provider attempt decluttering", () => {
	const attempt = (seq: number): AiActivityStep => ({
		seq,
		ts: `2026-04-12T07:15:${String(seq).padStart(2, "0")}.000Z`,
	...
	it("drops trailing attempts once done when other content exists", () => {
		const result = declutterProviderAttempts([content(1), attempt(2)], true);
