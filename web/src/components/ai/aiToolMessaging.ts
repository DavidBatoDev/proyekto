import type { AiActivityStepStatus, AiActivityStepTitleList } from "./types";

// =============================================================================
// Tool-name -> human copy catalog for the activity timeline. Lifted from
// `roadmap/ai/roadmapAiToolMessaging.ts`. The run runtime renamed the
// terminal tools (`stage_edits`, `propose`, `revise_proposal`); the old names
// stay in the catalog so persisted history rows keep their copy.
// =============================================================================
...
export const SUPPORTED_TRACE_TOOL_NAMES = [
	...
	"get_tasks_by_epic",
	// Run-era terminal tools. `propose` is last: text fallback matching is a
	// substring scan, and the bare word appears in ordinary prose.
	"stage_edits",
	"revise_proposal",
	// Cross-scope reads (workspace-scope sessions).
...
export type SupportedTraceToolName =
	(typeof SUPPORTED_TRACE_TOOL_NAMES)[number];
...
interface ToolMessageDescriptor {
	requestedTitle: string | string[];
	completedTitle: string | string[];
	buildCompletedTitle?: (ctx: ToolMessageContext) => string | string[];
	buildRequestedSummary: (ctx: ToolMessageContext) => string | string[];
	buildResultSummary: (ctx: ToolMessageContext) => string | string[];
}
...
export const seededRandomIndex = (seedString: string, max: number): number => {
	let hash = 0;
...
export const pickVariant = (
	...
	if (seed == null) return variants[0];
	const max = variants.length;
	if (typeof seed === "number") {
		return variants[Math.abs(Math.floor(seed)) % max];
	}
	return variants[seededRandomIndex(seed, max)];
};
...
const getResultTitleList = (
	ctx: ToolMessageContext,
): AiActivityStepTitleList | undefined => {
...
const getToolScopePhrase = (
	...
	switch (toolName) {
		...
		case "get_tasks_by_parent": {
			const parentScope =
				readParentScopePhrase(toolArgs) || "under the selected parent";
...
const buildToolResultContextSentence = (
	...
	if (scope) {
		return toSentence(
			`I checked ${scope} and confirmed the outcome${titleEvidence}`,
		);
	}
...
const maybeWarnUnknownTool = (toolName: string | null): void => {
	...
	console.warn("[AiToolMessaging] missing_tool_mapping", {
		tool_name: toolName,
	});
};

const TOOL_MESSAGE_CATALOG: Record<
	SupportedTraceToolName,
	ToolMessageDescriptor
> = {
	...
	resolve_node_reference: descriptor(
		...
		(ctx) =>
			outcomeTitle(
				ctx,
				"Found the right roadmap item",
				"No matching roadmap item found",
				"matches_count",
			),
	),
	search_nodes: descriptor(
		...
		(ctx) =>
			outcomeTitle(
				ctx,
				"Searched roadmap items",
				"No matching roadmap items found",
				"matches_count",
			),
	),
	get_workspace_overview: descriptor(
		"Reading the workspace overview",
		"Read the workspace overview",
		() =>
			"I am reading the projects, roadmaps, and teams you can access in this workspace.",
		(ctx) =>
			resultSummaryWithDefault(
				ctx,
				{},
				"I finished reading the workspace overview.",
			),
	),
	list_roadmaps: descriptor(
		"Listing roadmaps",
		"Listed roadmaps",
		(ctx) =>
			`I am listing the roadmaps you can access${readLimitPhrase(ctx.toolArgs)}.`,
		(ctx) =>
			resultSummaryWithDefault(
				ctx,
				{},
				"I finished listing the roadmaps you can access.",
			),
		(ctx) =>
			outcomeTitle(ctx, "Listed roadmaps", "No roadmaps found", "items_count"),
	),
	search_everything: descriptor(
		"Searching across roadmaps",
		"Searched across roadmaps",
		(ctx) =>
			`I am searching across your roadmaps for ${quote(toStringValue(ctx.toolArgs?.query)) ?? "your query"}${readLimitPhrase(ctx.toolArgs)}.`,
		(ctx) =>
			resultSummaryWithDefault(
				ctx,
				{ matches: "Found {count} matches" },
				"I finished searching across your roadmaps.",
			),
		(ctx) =>
			outcomeTitle(
				ctx,
	...
	list_my_tasks: descriptor(
		"Listing your tasks",
		"Listed your tasks",
		(ctx) =>
			`I am listing the tasks assigned to you${readLimitPhrase(ctx.toolArgs)}.`,
		(ctx) =>
			resultSummaryWithDefault(
				ctx,
				{ tasks: "Found {count} tasks" },
				"I finished listing the tasks assigned to you.",
			),
		(ctx) =>
	...
	list_project_members: descriptor(
		...
		() => "I am listing the members of the project.",
		(ctx) =>
			resultSummaryWithDefault(
				ctx,
				{},
				"I finished listing the project members.",
			),
	),
	...
	get_tasks_by_parent: descriptor(
		...
		(ctx) =>
			outcomeTitle(
				ctx,
				"Listed tasks under a parent item",
				"No tasks found under this item",
				"tasks_count",
			),
	),
	...
	get_blocked_items: descriptor(
		...
		(ctx) =>
			resultSummaryWithDefault(
				ctx,
				{
					matches: "Found {count} blocked items",
					tasks: "Found {count} blocked tasks",
				},
	...
	bulk_assign_tasks: descriptor(
		...
		(ctx) =>
			resultSummaryWithDefault(
				ctx,
				{
					operations: "Prepared {count} assignment changes",
					tasks: "Processed {count} tasks",
				},
	...
	bulk_update_feature_status: descriptor(
		...
		(ctx) => {
			...
			const status = humanizeEnum(toStringValue(ctx.toolArgs?.status));
			const scope =
				total > 0 ? `${total} selected features` : "selected features";
		...
		(ctx) =>
			resultSummaryWithDefault(
				...
				{
					operations: "Prepared {count} status changes",
					matches: "Processed {count} features",
				},
	...
	// Run-era terminal tools (same copy family as plan_roadmap_operations so
	// a thread with old and new rows reads consistently).
	stage_edits: descriptor(
		"Preparing your roadmap changes",
		"Prepared your roadmap changes",
		() => "I am putting together all the roadmap changes you requested.",
		(ctx) =>
	...
	propose: descriptor(
		"Drafting a proposal",
		"Drafted a proposal",
		() => "I am drafting a proposal for you to review before anything changes.",
		(ctx) =>
			resultSummaryWithDefault(
				ctx,
				{
					operations: "Outlined {count} changes",
					epics: "Outlined {count} epics",
				},
				"I drafted a proposal and it is ready for your review.",
			),
	),
	revise_proposal: descriptor(
		"Revising the proposal",
		"Revised the proposal",
		() => "I am updating the pending proposal with your feedback.",
		(ctx) =>
			resultSummaryWithDefault(
				ctx,
				{ operations: "Applied {count} revisions" },
				"I revised the proposal and it is ready for another look.",
			),
	),
};
...
export const extractTraceToolName = (step: {
	...
	const detailToolName = normalizeToolName(
		toStringValue(step.details?.tool_name),
	);
	if (detailToolName) return detailToolName;
	return (
		extractToolNameFromText(step.title) || extractToolNameFromText(step.summary)
	);
};
...
export const buildFriendlyMinimalToolLabel = (
	...
	const toolDescriptor =
		TOOL_MESSAGE_CATALOG[normalized as SupportedTraceToolName];
	if (!toolDescriptor) {
	...
	return {
		requested: pickVariant(toolDescriptor.requestedTitle, seed),
		completed: pickVariant(toolDescriptor.completedTitle, seed),
	};
};
...
export const buildCuratedToolRequestedMessage = (
	...
	return {
		title: pickVariant(toolDescriptor.requestedTitle, step.seq ?? step.summary),
		summary: pickVariant(
			toolDescriptor.buildRequestedSummary(toToolMessageContext(step)),
			step.seq ?? step.summary,
		),
		usedFallback: false,
	};
};
...
export const buildCuratedToolResultMessage = (
	...
	const descriptorSummary = pickVariant(
		toolDescriptor.buildResultSummary(ctx),
		step.seq ?? step.summary,
	);
	const titleList = getResultTitleList(ctx);
	const resolvedTitle = toolDescriptor.buildCompletedTitle
		...
		: pickVariant(toolDescriptor.completedTitle, step.seq ?? step.summary);
	return {
		title: resolvedTitle,
