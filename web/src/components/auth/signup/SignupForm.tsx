...
export function SignupForm(_props: SignupFormProps) {
	...
	const [phoneNumber, setPhoneNumber] = useState(() =>
		getStored("signup_phoneNumber"),
	);
	const [dateOfBirth, setDateOfBirth] = useState(() =>
		getStored("signup_dateOfBirth"),
	);
		...
		try {
			...
			if (authData.user) {
				try {
					...
					queryClient.setQueryData(profileKeys.byUser(authData.user.id), fresh);
			...
			// Signup flows always route through /welcome once; the welcome deck
			// performs the final handoff to conversion or dashboard.
		...
		<div style={{ width: "100%" }}>
			...
			{search.redirect?.includes("invites") && (
				...
				>
					...
					>
						<strong>You've been invited to join a project.</strong> Create an
