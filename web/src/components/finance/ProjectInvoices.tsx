...
export function ProjectInvoices({ projectId }: { projectId: string }) {
	...
	const [reverseRequest, setReverseRequest] = useState<{ invoice: Invoice; paymentId: string } | null>(null);
	...
	const paymentMutation = useMutation({
		mutationFn: ({ invoiceId, amount, paymentDate, method, reference, note }: { invoiceId: string; amount: number; paymentDate: string; method: string; reference: string; note: string }) =>
			invoiceService.recordPayment(invoiceId, { amount, payment_date: paymentDate, payment_method: method || undefined, reference: reference || undefined, note: note || undefined }),
		onSuccess: () => {
	...
	const voidMutation = useMutation({
		mutationFn: ({ invoiceId, reason }: { invoiceId: string; reason: string }) => invoiceService.voidAndReplace(invoiceId, reason),
		onSuccess: ({ replacement }) => {
			toast.success("Invoice voided; replacement draft created");
			setVoidInvoice(null);
			invalidate();
			void qc.invalidateQueries({ queryKey: ["finance"] });
			void navigate({ to: "/finance/invoices/$invoiceId/edit", params: { invoiceId: replacement.id }, search: { projectId } });
		},
		onError: (err) => toast.error((err as Error).message),
	});

	const reverseMutation = useMutation({
		mutationFn: ({ invoiceId, paymentId, reason }: { invoiceId: string; paymentId: string; reason: string }) => invoiceService.reversePayment(invoiceId, paymentId, reason),
		onSuccess: () => { toast.success("Payment reversed"); setReverseRequest(null); invalidate(); void qc.invalidateQueries({ queryKey: ["finance"] }); },
		onError: (err) => toast.error((err as Error).message),
	});
	...
	const outstanding = useMemo(() => {
		...
		for (const invoice of invoices) {
			...
			byCurrency.set(
				invoice.currency,
				(byCurrency.get(invoice.currency) ?? 0) + Number(invoice.balance_due ?? invoice.total ?? 0),
			);
		}
	...
	const rowProps = (invoice: Invoice) => ({
		...
		onReversePayment: (paymentId: string) => setReverseRequest({ invoice, paymentId }),
		isBusy:
		...
		<div className="w-full">
			...
			{paymentInvoice && (
				<PaymentModal
					...
					onConfirm={(data) => paymentMutation.mutate({ invoiceId: paymentInvoice.id, ...data })}
				/>
			)}
			{voidInvoice && (
				<VoidReplaceModal
					...
					onConfirm={(reason) => voidMutation.mutate({ invoiceId: voidInvoice.id, reason })}
				/>
			)}
			{reverseRequest && (
				<ReversePaymentModal isSaving={reverseMutation.isPending} onCancel={() => setReverseRequest(null)} onConfirm={(reason) => reverseMutation.mutate({ invoiceId: reverseRequest.invoice.id, paymentId: reverseRequest.paymentId, reason })} />
			)}
		</div>
	);
}
		...
		<div className="app-surface-card flex flex-col gap-4 p-4 md:flex-row md:items-center md:justify-between">
			<div className="min-w-0">
				...
				{!isDraft && invoice.status !== "void" && (
					<p className={`mt-0.5 text-xs font-medium ${invoice.is_overdue ? "text-destructive" : "text-muted-foreground"}`}>
						{invoice.is_overdue ? "Overdue" : "Balance due"}: {formatMoney(invoice.currency, Number(invoice.balance_due ?? invoice.total ?? 0))}
					</p>
				)}
				{invoice.payments.filter((payment) => !payment.reverses_payment_id).length > 0 && (
					<div className="mt-2 flex flex-wrap gap-1.5">
						{invoice.payments.filter((payment) => !payment.reverses_payment_id).map((payment) => {
							const reversed = invoice.payments.some((entry) => entry.reverses_payment_id === payment.id);
							return <span key={payment.id} className="inline-flex items-center gap-1 rounded-md bg-muted px-2 py-1 text-[11px] text-muted-foreground">{formatMoney(invoice.currency, payment.amount)} · {payment.payment_date}{reversed ? " · reversed" : <button type="button" onClick={() => onReversePayment(payment.id)} className="font-semibold text-primary hover:underline">Reverse</button>}</span>;
						})}
					</div>
				)}
			</div>
			...
			<div className="flex flex-wrap items-center gap-2">
				...
				>
					...
					{invoice.pdf_path ? (isDraft ? "Regenerate" : "Final PDF") : "Generate PDF"}
				</button>
				{invoice.pdf_path && (
					<button
						type="button"
				...
				{isDraft ? (
					...
					<>
						{canRecordPayment && (
							<button type="button" onClick={onRecordPayment} disabled={isBusy} className="inline-flex items-center gap-1 rounded-lg border border-border px-3 py-2 text-xs font-semibold text-foreground hover:bg-muted disabled:opacity-60">
								<CircleDollarSign className="h-3.5 w-3.5" /> Record payment
							</button>
						)}
						{invoice.status === "issued" && invoice.amount_paid === 0 && (
							<button type="button" onClick={onVoidAndReplace} disabled={isBusy} className="inline-flex items-center gap-1 rounded-lg border border-border px-3 py-2 text-xs font-semibold text-muted-foreground hover:border-destructive hover:text-destructive disabled:opacity-60">
...
function PaymentModal({
	...
	isSaving: boolean;
	onCancel: () => void;
	onConfirm: (data: { amount: number; paymentDate: string; method: string; reference: string; note: string }) => void;
}) {
	...
	return (
		<div className="fixed inset-0 z-100 flex items-center justify-center bg-black/40 p-4">
			<form onSubmit={(event) => { event.preventDefault(); onConfirm({ amount: Number(amount), paymentDate, method, reference, note }); }} className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-xl">
				<h2 className="text-base font-bold text-card-foreground">Record payment</h2>
				<p className="mt-1 text-xs text-muted-foreground">{invoice.number} has {formatMoney(invoice.currency, invoice.balance_due)} remaining.</p>
				<div className="mt-4 grid gap-3">
					<label className="text-xs font-semibold text-muted-foreground">Amount<input required min="0.01" max={invoice.balance_due} step="0.01" type="number" value={amount} onChange={(event) => setAmount(event.target.value)} className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" /></label>
					<label className="text-xs font-semibold text-muted-foreground">Payment date<input required type="date" value={paymentDate} onChange={(event) => setPaymentDate(event.target.value)} className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" /></label>
					<label className="text-xs font-semibold text-muted-foreground">Method<input value={method} onChange={(event) => setMethod(event.target.value)} placeholder="Bank transfer, GCash…" className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" /></label>
					<label className="text-xs font-semibold text-muted-foreground">Reference<input value={reference} onChange={(event) => setReference(event.target.value)} className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" /></label>
					<label className="text-xs font-semibold text-muted-foreground">Note<textarea value={note} onChange={(event) => setNote(event.target.value)} className="mt-1.5 min-h-18 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" /></label>
				</div>
				<div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onCancel} disabled={isSaving} className="rounded-lg border border-border px-3 py-2 text-xs font-semibold text-muted-foreground">Cancel</button><button type="submit" disabled={isSaving || !(Number(amount) > 0)} className="app-cta rounded-lg px-3.5 py-2 text-xs font-semibold text-white">{isSaving ? "Saving…" : "Record payment"}</button></div>
			</form>
		</div>
	);
}

function VoidReplaceModal({ invoice, isSaving, onCancel, onConfirm }: { invoice: Invoice; isSaving: boolean; onCancel: () => void; onConfirm: (reason: string) => void }) {
	const [reason, setReason] = useState("");
	return <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/40 p-4"><form onSubmit={(event) => { event.preventDefault(); onConfirm(reason); }} className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-xl"><h2 className="text-base font-bold text-card-foreground">Void and replace {invoice.number}?</h2><p className="mt-1 text-xs leading-relaxed text-muted-foreground">This preserves the issued invoice for audit purposes and creates a new editable draft with copied client-facing details.</p><label className="mt-4 block text-xs font-semibold text-muted-foreground">Reason<textarea required value={reason} onChange={(event) => setReason(event.target.value)} className="mt-1.5 min-h-22 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" /></label><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onCancel} disabled={isSaving} className="rounded-lg border border-border px-3 py-2 text-xs font-semibold text-muted-foreground">Cancel</button><button type="submit" disabled={isSaving || !reason.trim()} className="rounded-lg bg-destructive px-3.5 py-2 text-xs font-semibold text-white">{isSaving ? "Voiding…" : "Void & create draft"}</button></div></form></div>;
}

function ReversePaymentModal({ isSaving, onCancel, onConfirm }: { isSaving: boolean; onCancel: () => void; onConfirm: (reason: string) => void }) {
	const [reason, setReason] = useState("");
	return <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/40 p-4"><form onSubmit={(event) => { event.preventDefault(); onConfirm(reason); }} className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-xl"><h2 className="text-base font-bold text-card-foreground">Reverse payment?</h2><p className="mt-1 text-xs text-muted-foreground">The original payment stays in the audit trail and the invoice balance will be recalculated.</p><label className="mt-4 block text-xs font-semibold text-muted-foreground">Reason<textarea required value={reason} onChange={(event) => setReason(event.target.value)} className="mt-1.5 min-h-22 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" /></label><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onCancel} disabled={isSaving} className="rounded-lg border border-border px-3 py-2 text-xs font-semibold text-muted-foreground">Cancel</button><button type="submit" disabled={isSaving || !reason.trim()} className="rounded-lg bg-destructive px-3.5 py-2 text-xs font-semibold text-white">{isSaving ? "Reversing…" : "Reverse payment"}</button></div></form></div>;
}
