function PulseBlock({
	...
	return (
		<div
			className={`${className} bg-gray-200 ${animated ? "animate-pulse" : ""}`}
		/>
	);
}
				...
				<div className="px-3 pb-4">
					...
					<div className="space-y-2">
						{Array.from({ length: 7 }).map((_, index) => (
							<div
								key={`chat-sidebar-skeleton-${index}`}
...
export function ChatCenterShellSkeleton({
	animated = true,
}: {
	animated?: boolean;
}) {
			...
			<div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-3 py-4 md:px-6 md:py-5">
				<div className="w-full max-w-4xl mr-auto min-w-0 space-y-4">
					{Array.from({ length: 9 }).map((_, index) => (
						<div
							key={`chat-center-skeleton-${index}`}
						...
						>
							...
							<div className="flex-1 min-w-0 space-y-2 pt-1">
								<PulseBlock
									className="h-3.5 w-48 rounded"
									animated={animated}
								/>
								<PulseBlock
									className="h-3 w-[82%] rounded"
									animated={animated}
								/>
								<PulseBlock
...
export function ChatProfilePanelSkeleton({
	animated = true,
}: {
	animated?: boolean;
}) {
			...
			<div className="flex-1 min-h-0 overflow-y-auto px-3 pt-3 pb-4">
				...
				<div className="rounded-xl border border-gray-200 bg-white p-1.5 space-y-1.5">
					{Array.from({ length: 8 }).map((_, index) => (
						<div
							key={`chat-profile-skeleton-${index}`}
