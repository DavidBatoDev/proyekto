...
function groupChannelMembers(members: ChatParticipant[]): ChannelMemberGroup[] {
	const groups = new Map<string, ChannelMemberGroup>();

	for (const member of members) {
		const team = member.team ?? null;
		const key = team ? `team:${team.id}` : "project-members";
		const group = groups.get(key) ?? {
			key,
			label: team?.name ?? "Project members",
			team,
			members: [],
		};

		if (!group.members.some((item) => item.user_id === member.user_id)) {
...
export function ChatInfoPanel({
	...
	const libraryQuery = useRoomLibraryQuery(
		roomId ?? "",
	...
	const groupedChannelMembers = useMemo(
		() => groupChannelMembers(sortedChannelMembers),
		[sortedChannelMembers],
	);
												...
												{groupedChannelMembers.map((group) => (
													<section key={group.key}>
														<div className="flex items-center gap-2 px-2 pb-1 pt-1">
															{group.team ? (
																<TeamAvatar team={group.team} size="sm" />
															) : (
															...
															return (
																...
																>
																	<ChatAvatar
																		name={label}
																		avatarUrl={
																			participant.user?.avatar_url ?? null
																		}
