...
export function TeamPage({ projectId }: { projectId: string }) {
  ...
  // row that hasn't been backfilled yet.
  const directShares = members.filter((m) =>
    m.has_direct_grant !== undefined
      ? m.has_direct_grant
      : !m.origin || !m.origin.startsWith("team:"),
  );
  ...
  // Curated members per attached team — the structural source of
  // "this user is on the project via this team." Used to render
  // "Also on <Team>" chips next to direct collaborators.
  const curatedQueries = useQueries({
    queries: projectTeams.map((pt) => ({
      queryKey: ["project", projectId, "teams", pt.team_id, "curated"],
  ...
  const directAlsoOnByUserId = useMemo(() => {
    ...
    for (let i = 0; i < projectTeams.length; i++) {
      const teamId = projectTeams[i].team_id;
      const name = teamNameById.get(teamId);
      if (!name) continue;
      const curated = curatedQueries[i]?.data ?? [];
      for (const c of curated) {
        if (!directUserIds.has(c.user_id)) continue;
    ...
    return out;
  }, [directShares, projectTeams, curatedQueries, teamNameById]);
...
function AttachedTeamCard({
  ...
  const curatedQuery = useQuery({
    queryKey: ["project", projectId, "teams", projectTeam.team_id, "curated"],
  ...
  const sortedCurated = useMemo(() => {
    ...
    return [...curated].sort((a, b) => {
      const roleA = syncedRoleByUserId.get(a.user_id) ?? "viewer";
      const roleB = syncedRoleByUserId.get(b.user_id) ?? "viewer";
      const rankDiff = roleRank(roleB) - roleRank(roleA);
      if (rankDiff !== 0) return rankDiff;
      const nameA = (a.user?.display_name ?? a.user_id).toLowerCase();
      const nameB = (b.user?.display_name ?? b.user_id).toLowerCase();
      return nameA.localeCompare(nameB);
    });
  }, [curated, syncedRoleByUserId]);
            ...
            <ul className="divide-y divide-slate-200">
              {sortedCurated.map((m) => {
                ...
                const outranked =
                  ...
                  memberHasGate(targetMember, "members.manage");
                return (
                  <CuratedMemberRow
                    key={m.user_id}
                    member={m}
                    ...
                    canManage={canManage && !isSelf && !outranked}
                    isDirect={directUserIds.has(m.user_id)}
                    isRemoving={
        ...
        <ul className="divide-y divide-slate-200">
          {shares.map((m) => {
            const isSelf =
              currentUserId !== null && m.user_id === currentUserId;
            const outranked =
              !callerIsOwner && memberHasGate(m, "members.manage");
            return (
              <ProjectMemberRow
                key={m.id}
                ...
                alsoOnLabels={
                  m.user_id ? alsoOnByUserId.get(m.user_id) : undefined
                }
                onRemove={
                  canManage && !isSelf && !outranked
                    ? () =>
