...
export function ProjectBottomNav({
  ...
  const hasUnreadChat = useMemo(() => {
    ...
    const roomHasUnread = (room: ChatRoom): boolean => {
      ...
      const viewerLastReadAt =
        ...
        room.participants.find((p) => p.user_id === currentUserId)?.last_read_at ??
        null;
      if (!viewerLastReadAt) {
        return currentUserId ? room.last_message.sender_id !== currentUserId : true;
      }
      return (
  ...
  const isMoreActive =
    isProjectActive &&
    moreItems.some((item) => item.isActive);
