			...
			features: [
				{
					...
					description:
						"Create secure onboarding, sessions, user profiles, and role-aware access.",
					taskCount: 5,
				...
				},
				{
					...
					description:
						"Design core entities, API contracts, environments, and automated deploys.",
					taskCount: 6,
				...
				},
				{
					...
					description:
						"Create responsive navigation, shared components, and product-ready layouts.",
					taskCount: 4,
			...
			features: [
				{
					...
					description:
						"Let users create routines with exercises, sets, reps, rest timers, and notes.",
					taskCount: 7,
					tasks: [
						"Exercise catalog",
						"Routine templates",
				...
				},
				{
					...
					description:
						"Surface workout history, streaks, personal records, and weekly summaries.",
					taskCount: 5,
				...
				},
				{
					...
					description:
						"Track goals, meals, measurements, and lightweight health progress signals.",
					taskCount: 5,
			...
			features: [
				{
					...
					description:
						"Add QA coverage, observability, seed data, support flows, and launch checklist.",
					taskCount: 6,
				...
				},
				{
					...
					description:
						"Ship reminders, achievements, referral hooks, pricing tests, and analytics.",
					taskCount: 5,
					tasks: [
						"Workout reminders",
						"Achievement badges",
				...
				},
				{
					...
					description:
						"Prepare admin tools, feedback loops, customer support, and launch reporting.",
					taskCount: 4,
...
const DEMO_TURNS: DemoTurn[] = [
	...
	{
		prompt:
			"Add the core product experience for workouts and progress tracking.",
		response:
...
function RoadmapStructurePanel({
	...
	const taskCount = features.reduce(
		(sum, feature) => sum + feature.taskCount,
		0,
	);
									...
									{epic.features.map((feature, featureIndex) => {
										...
										return (
											...
											>
												...
												<span
													className={`rounded-md border px-1.5 py-0.5 ${status.featureBadge}`}
												>
													{status.featureLabel}
												</span>
...
function FlowFeatureNode({
	...
	return (
		...
		>
			...
			<span
				className={`inline-flex rounded border px-2 py-0.5 text-[9px] font-medium ${status.featureBadge}`}
			>
				{status.featureLabel}
			</span>
			...
			<div className="space-y-1">
				{feature.tasks.slice(0, 3).map((task) => (
					<div key={task} className="flex items-center gap-1.5">
						...
						<span
							className={`rounded px-1.5 py-0.5 text-[7px] font-medium ${status.taskBadge}`}
						>
							{status.taskLabel}
						</span>
					</div>
	...
	return (
		...
		>
			...
			<div className="absolute right-3 top-3 z-20 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
				<button
					type="button"
					className="flex h-9 w-9 items-center justify-center border-b border-slate-200"
				>
			...
			<div className="absolute bottom-3 left-1/2 z-20 hidden -translate-x-1/2 items-center gap-2 rounded-2xl border border-slate-200 bg-white/95 px-3 py-2 text-xs text-slate-600 shadow-xl xl:flex">
				...
				<span className="rounded-xl border border-slate-200 px-3 py-1.5 font-medium">
					Epic
				</span>
				<span className="rounded-xl border border-slate-200 px-3 py-1.5 font-medium">
					Feature
					...
					<>
						<svg className="absolute inset-0 h-full w-full" aria-hidden="true">
							{visibleEpics.slice(0, -1).map((epic, index) => {
								const nextStatus =
									STATUS_STYLES[(index + 1) % STATUS_STYLES.length];
							...
							{visibleEpics.flatMap((epic, epicIndex) =>
								epic.features.map((feature, featureIndex) => {
									const status =
										STATUS_STYLES[
											(epicIndex + featureIndex) % STATUS_STYLES.length
										];
									const epicY =
										getEpicTop(epicIndex) + FLOW_LAYOUT.epicHeight / 2;
									const featureY =
										getFeatureTop(epicIndex, featureIndex) +
										FLOW_LAYOUT.featureHeight / 2;
									return (
										<motion.path
...
export function AIDemoSection({
	...
	useEffect(() => {
		...
		DEMO_TURNS.forEach((turn, turnIndex) => {
			...
			t(elapsed + 280, () => {
				setSubmittedPrompts(
					DEMO_TURNS.slice(0, turnIndex + 1).map((item) => item.prompt),
				);
				setTypedText("");
				setPhase("thinking");
			});
			...
			t(elapsed + 1250, () => {
				...
				setAiResponses(
					DEMO_TURNS.slice(0, turnIndex + 1).map((item) => item.response),
				);
				setPhase("done");
			});
									...
									{phase !== "idle" && (
										...
										>
											...
											<div className="rounded-2xl rounded-tl-sm border border-slate-200 bg-white px-3 py-2 text-sm leading-relaxed text-slate-700 shadow-sm">
												Hi! Tell me what you're building and I'll create a
									...
									<motion.div
										animate={
											phase === "thinking"
												? {
														scale: [1, 1.18, 1],
														backgroundColor: ["#5E6AD2", "#8B93E2", "#5E6AD2"],
													}
												: {}
										}
