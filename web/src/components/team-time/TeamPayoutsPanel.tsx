...
import type { TeamLogsSearch } from "@/components/team-time/TeamLogsPanel";
import {
	formatMoney,
	initialsFromName,
	logFee,
...
export interface TeamPayoutsPanelLinks {
	/**
	 * Open Team Logs with the given search (a resolved cut-off period, status
	 * "pending", and optionally a member). Backs the per-member "Review"
	 * button; omit it and that button is hidden (Pay stays locked while the
	 * member has pending logs).
	 */
	openTeamLogs?: (search: TeamLogsSearch) => void;
}
...
export function TeamPayoutsPanel({ teamId, links }: TeamPayoutsPanelProps) {
	const qc = useQueryClient();
	const openTeamLogs = links?.openTeamLogs;
					...
					{approvedQuery.isPending || pendingQuery.isPending ? (
						...
						groups.map((g) => (
							<CutoffSection
								...
								onReview={
									openTeamLogs
										? (memberId) => handleReview(g, memberId)
										: undefined
								}
							/>
						))
...
function CutoffSection({
	...
	/** Omitted when the host gives no way to reach Team Logs. */
