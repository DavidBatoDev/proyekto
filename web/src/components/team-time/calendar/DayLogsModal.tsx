...
export function DayLogsModal({
	...
	return (
		<AppDialog
			...
			title={dateLabel}
			description={`${logs.length} log${logs.length === 1 ? "" : "s"}`}
			footer={
				<>
					...
					{canBulkAct && selectionInfo.count > 0 && (
						<>
							<span className="mr-auto px-1 text-xs font-semibold text-muted-foreground">
					...
					<button
						type="button"
						onClick={onClose}
						className="rounded-md border border-border bg-card px-3 py-2 text-xs font-semibold text-foreground hover:bg-muted"
					>
						Close
					</button>
				</>
