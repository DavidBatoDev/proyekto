...
interface TeamTaskTreePickerProps {
	tasks: ProjectTaskOption[];
	value: string;
	disabled?: boolean;
	placeholder?: string;
...
export function TeamTaskTreePicker({
	tasks,
	value,
	disabled = false,
	...
	const groups = useMemo<EpicGroup[]>(() => {
		const epicMap = new Map<
			string,
			{
				featureMap: Map<string, { tasks: ProjectTaskOption[] }>;
			}
		>();
	...
	useEffect(() => {
		...
		for (const epicGroup of groups) {
			...
			for (const featureGroup of epicGroup.features) {
				nextExpandedFeatures[
					`${epicGroup.epicTitle}::${featureGroup.featureTitle}`
				] = true;
			}
		}
	...
	const renderGuidedPicker = () => {
		if (groups.length === 0) {
			return (
				<p className="px-2 py-2 text-xs text-gray-500">No tasks available.</p>
			);
		}
				...
				<>
					...
					{focusedEpicGroup.features.map((featureGroup) => (
						<button
							...
							type="button"
							onClick={() =>
								setSelectedFeatureFocus(featureGroup.featureTitle)
							}
