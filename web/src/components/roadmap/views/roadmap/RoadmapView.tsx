...
const CANVAS_SKIP_FEATURE_REORDER_KEY =
	"roadmap.canvas.skipFeatureReorderConfirm";
...
export const RoadmapView = ({
	...
	const nodes = useMemo((): Node<EpicWidgetData | FeatureWidgetData>[] => {
	...
	const computeReorderedEpics = useCallback(
		(
			...
			const featureNodes = currentNodes.filter(
				(n) => n.type === "featureWidget",
			);
			...
			for (let index = 0; index < epicOrder.length; index++) {
				...
				for (let i = 0; i < originalFeatures.length; i++) {
					...
					const featureCenterY = featureNode
						? featureNode.position.y +
							((featureNode.height as number) ?? 150) / 2
						: Infinity;
	...
	const computeDragPreview = useCallback(
		(args: {
			...
			const { ds, draggedPosition, baseNodes, originalNodes, relativeYs } =
				args;
	...
	const handleCanvasDragOver = useCallback(
		(event: DragEvent<HTMLDivElement>) => {
			...
			const candidateNodes =
				(workingNodesRef.current as
					| Node<EpicWidgetData | FeatureWidgetData>[]
					| null) ?? reactFlowInstance.getNodes();
