			...
			sortedMilestones.map((milestone) => {
				...
				return {
					...
					target_date: new Date(
						`${draftTargetDate}T00:00:00.000Z`,
					).toISOString(),
				};
			}),
	...
	const queueFeatureDatePersist = useCallback(
		(change: FeatureDateDraftCommit) => {
			...
			const previousTimeout =
				featureDatePersistTimeoutsRef.current.get(featureId);
			if (previousTimeout) {
		...
		setEpicDateVisualDrafts((prev) => {
			...
			for (const [epicId, draft] of Object.entries(prev)) {
				...
				if (!epic?.start_date || !epic?.end_date) {
					...
					epic.start_date === draft.startDate &&
					epic.end_date === draft.endDate
				) {
					delete next[epicId];
					changed = true;
				}
	...
	const handleEpicDateDraftCommit = useCallback(
		(change: EpicDateDraftCommit) => {
			...
			setPendingDateChange({
				kind: "epic",
				change,
				payload: toEpicConfirmPayload(change),
			});
		},
		[
			canEditTimelineDates,
									...
									{canEditTimelineDates && (
										<>
											...
											<button
												...
												title={
													isDateDrawMode ? "Exit draw mode" : "Draw date ranges"
												}
