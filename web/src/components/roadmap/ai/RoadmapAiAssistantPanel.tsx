        ...
        row.role === "user" ? (
          ...
          <div key={i} className="ml-0 mr-4 px-0 py-1.5 space-y-2">
            <div className="flex items-center gap-1.5 mb-1">
              <SkeletonBlock className="h-2 w-12 gemini-shimmer" />
            </div>
            {row.lines.map((w, j) => (
              <SkeletonBlock
                key={j}
                className="h-2.5 gemini-shimmer"
                style={{ width: `${w}%` }}
              />
            ))}
          </div>
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-3 bg-slate-900/40 relative [scrollbar-width:thin] [scrollbar-color:rgba(156,163,175,0.5)_transparent] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-600 [&::-webkit-scrollbar-thumb]:rounded-full hover:[scrollbar-color:rgba(107,114,128,0.7)_transparent] hover:[&::-webkit-scrollbar-thumb]:bg-slate-9000">
        ...
        !isLiveTimelineAnchoredInMessage ? (
          ...
          <div className="mr-4 space-y-2 py-1" aria-label="Assistant is thinking">
            <div className="gemini-shimmer h-3 w-[92%] rounded-md" />
            <div className="gemini-shimmer h-3 w-[78%] rounded-md" />
            <div className="gemini-shimmer h-3 w-[55%] rounded-md" />
          </div>
        ) : null}
