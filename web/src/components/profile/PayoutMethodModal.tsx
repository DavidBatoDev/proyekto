...
export function PayoutMethodModal({
	...
	return (
		<div
			className="fixed inset-0 z-165 flex items-center justify-center bg-primary/55 p-4 backdrop-blur-[2px]"
			onClick={onClose}
		>
			<div
				className="max-h-[92vh] w-full max-w-md overflow-y-auto rounded-2xl border border-border bg-card shadow-2xl"
			...
			>
						...
						{qrPreview ? (
							<div className="flex items-start gap-3">
								<img
									src={qrPreview}
									alt="Payout QR"
									className="h-28 w-28 rounded-lg border border-border bg-card object-contain p-1"
								/>
							...
							<label className="flex cursor-pointer items-center gap-2 rounded-lg border border-dashed border-input px-3 py-3 text-xs text-muted-foreground hover:bg-muted/40">
				...
				<div className="flex items-center justify-end gap-2 border-t border-border bg-muted/40 px-5 py-4">
					<button
						...
						onClick={onClose}
						disabled={isSaving}
						className="inline-flex items-center gap-1.5 rounded-md border border-input bg-card px-3 py-2 text-xs font-semibold text-foreground hover:bg-muted disabled:opacity-50"
					>
