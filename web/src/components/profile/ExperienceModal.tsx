...
export function ExperienceModal({
	...
	const set = (k: keyof ExpPayload, v: unknown) =>
		setForm((p) => ({ ...p, [k]: v }));
	...
	return (
		<ProfileModal
			isOpen={isOpen}
			onClose={handleClose}
			title={isEdit ? "Edit Experience" : "Add Work Experience"}
		>
				...
				<div className="grid grid-cols-2 gap-3">
					<div>
						...
						<input
							...
							onChange={(e) =>
								set("start_date", e.target.value ? e.target.value + "-01" : "")
							}
							className={cls}
						/>
					</div>
					<div>
						...
						<input
							...
							onChange={(e) =>
								set("end_date", e.target.value ? e.target.value + "-01" : null)
							}
