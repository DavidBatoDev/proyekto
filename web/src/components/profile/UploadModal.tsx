	...
	const validate = useCallback(
		(file: File): string | undefined => {
			if (
				!acceptTypes.some(
					(t) =>
						file.type === t || file.name.endsWith(t.replace("image/", ".")),
				)
			) {
	...
	return (
		...
		>
			<div className="space-y-4">
				...
				{entries.length > 0 && (
					<div
						className={`grid gap-3 ${maxFiles === 1 ? "grid-cols-1" : "grid-cols-2 sm:grid-cols-3"}`}
					>
						{entries.map((entry, idx) => (
							<div
								key={idx}
								className={`relative rounded-xl overflow-hidden border-2 transition-colors ${entry.error ? "border-red-300" : "border-green-300"}`}
							>
								...
								{/* Status badge */}
								<div
									className={`absolute top-1.5 left-1.5 rounded-full p-0.5 ${entry.error ? "bg-red-500" : "bg-green-500"}`}
								>
									{entry.error ? (
