	...
	return (
		<div className="relative">
			<select
				...
				className={`text-xs font-medium px-2 py-0.5 rounded-full border-0 appearance-none pr-5 cursor-pointer focus:outline-none focus:ring-1 focus:ring-primary/40 ${current.color}`}
			>
			...
			<div className="space-y-6">
				...
				<div>
					...
					<textarea
						...
						rows={5}
						maxLength={2000}
						className="w-full px-3 py-2 border border-input rounded-lg text-sm leading-relaxed focus:outline-none focus:ring-2 focus:ring-primary/40 resize-y"
						...
						<div className="flex flex-wrap gap-2 mb-3">
							{skills.map((s) => {
								return (
									<div
										key={s.skill_id}
										className="flex items-center gap-1.5 pl-3 pr-1 py-1 bg-muted rounded-full group"
									>
										...
										<button
											...
											className="w-4 h-4 flex items-center justify-center rounded-full text-muted-foreground hover:text-destructive hover:bg-red-50 transition-colors"
										>
											<X className="w-3 h-3" />
										</button>
									</div>
							...
							<div className="max-h-56 overflow-y-auto">
								{loadingMeta ? (
									...
									Array.from(grouped.entries()).map(([category, items]) => (
										<div key={category}>
											<p className="px-3 pt-2.5 pb-1 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
												{category}
											</p>
											{items.map((skill) => (
												<button
													...
													className="w-full text-left px-4 py-2 text-sm text-foreground hover:bg-primary/5 hover:text-primary transition-colors flex items-center gap-2"
												>
