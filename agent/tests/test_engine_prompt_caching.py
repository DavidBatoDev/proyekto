...
class PromptCacheKeyTests(unittest.TestCase):
    def _client(self, cache_key):
        ...
        client = LLMClient(settings, prompt_cache_key=cache_key)
...
class OutcomeCarriesCachedTokensTests(unittest.TestCase):
    ...
    def test_loop_result_cached_tokens_reach_the_step_result(self):
        from app.core.runtime.results import StepResult

        self.assertIn('tokens_cached', StepResult.__dataclass_fields__)

    def test_step_context_and_finalize_forward_cached_tokens(self):
        import inspect

        from app.core.runtime import orchestrator, service

        self.assertIn("self.tokens['cached'] +=", inspect.getsource(service.StepContext.add_loop_usage))
        self.assertIn("tokens_cached=ctx.tokens['cached']", inspect.getsource(orchestrator.finalize_step))
