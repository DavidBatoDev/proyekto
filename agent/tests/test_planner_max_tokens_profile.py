...
class PlannerMaxTokensProfileTests(unittest.TestCase):
    def test_default_profile_uses_edit_default_max_tokens(self) -> None:
        adapter = _build_adapter()
        self.assertEqual(
            adapter._planner_max_tokens_for_profile(None),
            adapter._settings.openai_edit_default_max_tokens,
        )
    ...
    def test_scoped_profile_uses_narrow_edit_max_tokens(self) -> None:
        ...
        self.assertEqual(
            ...
            adapter._settings.openai_edit_narrow_max_tokens,
        )
    ...
    def test_repair_profile_uses_repair_max_tokens(self) -> None:
        ...
        self.assertEqual(
            ...
            adapter._settings.openai_edit_repair_max_tokens,
        )

    def test_narrow_edit_max_tokens_default_is_800(self) -> None:
        self.assertEqual(get_settings().openai_edit_narrow_max_tokens, 800)

    def test_legacy_planner_env_aliases_still_accepted(self) -> None:
        # Renamed OPENAI_PLANNER_* → OPENAI_EDIT_*; AliasChoices keeps the
        # old env vars working for one release so existing deployments don't
        # break silently.
        from app.core.config import Settings

        legacy_max = Settings(_env_file=None, OPENAI_PLANNER_MAX_TOKENS=1500)  # type: ignore[arg-type]
        self.assertEqual(legacy_max.openai_edit_default_max_tokens, 1500)
        legacy_default = Settings(_env_file=None, OPENAI_PLANNER_DEFAULT_MAX_TOKENS=1700)  # type: ignore[arg-type]
        self.assertEqual(legacy_default.openai_edit_default_max_tokens, 1700)
        legacy_repair = Settings(_env_file=None, OPENAI_PLANNER_REPAIR_MAX_TOKENS=2200)  # type: ignore[arg-type]
        self.assertEqual(legacy_repair.openai_edit_repair_max_tokens, 2200)
        legacy_narrow = Settings(_env_file=None, OPENAI_PLANNER_NARROW_EDIT_MAX_TOKENS=600)  # type: ignore[arg-type]
        self.assertEqual(legacy_narrow.openai_edit_narrow_max_tokens, 600)
