"""Upstream (NestJS) error-payload parsing for the session routes.

The backend's error bodies come in several nestings (``{detail: {error: {...}}}``
via the agent's own HTTPException envelope, bare ``{code, message}``, Nest's
``{statusCode, error, message}``). These helpers flatten them into one shape the
routes and the auto-commit path can switch on.
"""
