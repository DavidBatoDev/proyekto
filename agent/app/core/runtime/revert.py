...
def filter_history_by_roadmap(
    history: list[ChangeGroup], roadmap_id: str | None
) -> list[ChangeGroup]:
    """Groups that belong to ``roadmap_id`` (order preserved). Groups recorded
    before roadmaps were tracked (``roadmap_id is None``) are kept so a
    pre-migration history stays revertible; ``roadmap_id=None`` keeps all."""
    if not roadmap_id:
        return list(history)
    return [
        group for group in history
        if group.roadmap_id is None or group.roadmap_id == roadmap_id
    ]


def roadmap_ids_with_history(history: list[ChangeGroup]) -> list[str]:
    """Distinct roadmap ids in the history (most-recent-first), for the
    ``REVERT_NEEDS_ROADMAP`` listing."""
    seen: list[str] = []
    for group in history:
        if group.roadmap_id and group.roadmap_id not in seen:
            seen.append(group.roadmap_id)
    return seen


def select_revert_range(
    history: list[ChangeGroup],
    change_id: str | None,
    *,
    ...
    ``roadmap_id`` narrows the history to that roadmap's groups first (a
    revert is one commit on one roadmap). ``change_id=None`` -> just the
    latest group. Otherwise the latest group down to (and including) the
    group with that ``change_id``. Returns [] when history is empty or the
    change_id isn't found.
    """
    scoped = filter_history_by_roadmap(history, roadmap_id)
    if not scoped:
        return []
    if change_id is None:
        return [scoped[0]]
    target = change_id.strip()
    for index, group in enumerate(scoped):
