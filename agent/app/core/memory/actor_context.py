...
def ensure_actor_context(
    *,
    ...
    # Roadmap scope keeps the roadmap-keyed actor route (it carries the
    # roadmap role); workspace scope has no focus roadmap and reads the
    # user-scoped GET /ai/context/actor instead (no role).
    focus_roadmap_id = session.scope.focus_roadmap_id
    try:
        if focus_roadmap_id:
            actor_coro = nest_client.context_actor(
                roadmap_id=focus_roadmap_id,
                auth_header=auth_header,
                trace_id=trace_id,
            )
        else:
            actor_coro = nest_client.ai_context_actor(auth_header, trace_id=trace_id)
        actor_payload = run_async_call(actor_coro)
