...
from app.core.memory.pending_plan_revision_applier import (
...
def _roadmap_anchor(
    session: AgentSession, roadmap_id: str | None = None
) -> tuple[int | None, str | None, str | None]:
    """``(base_revision, revision_token, overview_hash)`` for a roadmap: from
    its ``RoadmapContext`` when loaded, else the session's legacy focus mirrors
    (``AgentSession.base_revision`` / ``revision_token``) for the focus roadmap
    and nothing for an unloaded non-focus roadmap."""
    focus = session.scope.focus_roadmap_id
    target = roadmap_id or focus
    context = session.metadata.roadmaps.get(target) if target else None
    is_focus = target is None or target == focus
    if context is not None:
        base_revision = context.base_revision
        if base_revision is None and is_focus:
            base_revision = session.base_revision
        token = context.revision_token or (session.revision_token if is_focus else None)
        return base_revision, token, _compute_overview_hash(context.overview_summary)
    if is_focus:
        return session.base_revision, session.revision_token, None
    return None, None, None


def _stamp_target_anchors(session: AgentSession, plan: PendingPlan) -> None:
    """Fill each target's staleness anchors (and derived op counts) from the
    session's per-roadmap context."""
    for target in plan.targets:
        base_revision, token, overview_hash = _roadmap_anchor(session, target.roadmap_id)
        if target.base_revision is None:
            target.base_revision = base_revision
        if not target.revision_token:
            target.revision_token = token
        if not target.overview_hash:
            target.overview_hash = overview_hash
        if target.operations:
            if not target.operations_count:
                target.operations_count = len(target.operations)
            if not target.contains_delete and any(
                str(getattr(op.op, 'value', op.op)) == 'delete_node' for op in target.operations
            ):
                target.contains_delete = True
        if target.roadmap_title is None:
            context = session.metadata.roadmaps.get(target.roadmap_id)
            if context is not None and context.title:
                target.roadmap_title = context.title
...
def _plan_has_content(plan: PendingPlan) -> bool:
    """A proposal is recordable when it carries titles (kind='plan') or
    concrete operations on at least one target (kind='edits'). Delete-only /
    update-only edits proposals legitimately have an empty hierarchy."""
    if plan.kind == 'edits':
        return any(target.operations for target in plan.targets) or bool(
            plan.proposed_hierarchy
        ) or any(target.proposed_hierarchy for target in plan.targets)
    return bool(plan.proposed_hierarchy) or any(
        target.proposed_hierarchy for target in plan.targets
    )
...
def _record_pending_plan_question(
    ...
    *,
    ...
    base_revision, revision_token, overview_hash = _roadmap_anchor(session)
    plan_kwargs: dict[str, Any] = {
        ...
        'roadmap_overview_hash': overview_hash,
    }
    run_id = payload.get('run_id')
    if isinstance(run_id, str) and run_id.strip():
        plan_kwargs['run_id'] = run_id.strip()
    elif existing is not None and existing.run_id:
        plan_kwargs['run_id'] = existing.run_id
...
def _record_pending_plan_final(
    ...
    *,
    ...
    # A revision of a multi-target plan arrives with the revised hierarchy
    # only: carry the prior targets forward, the first one taking the new
    # hierarchy (revise_proposal edits the legacy card's hierarchy).
    if (
        not payload_for_model.get('targets')
        and existing is not None
        ...
        carried = [
            target.model_dump(mode='json', exclude_none=True) for target in existing.targets
        ]
        revised_hierarchy = payload_for_model.get('proposed_hierarchy')
        if isinstance(revised_hierarchy, list):
            carried[0]['proposed_hierarchy'] = revised_hierarchy
        payload_for_model['targets'] = carried
        payload_for_model.setdefault('kind', existing.kind)
    ...
    # `proposed_hierarchy` mirrors targets[0] for the legacy single-roadmap
    # card (and vice versa is left to the propose phase, which builds targets).
    if plan.targets and not plan.proposed_hierarchy and plan.targets[0].proposed_hierarchy:
        plan.proposed_hierarchy = [
            epic.model_copy(deep=True) for epic in plan.targets[0].proposed_hierarchy
        ]

    # Reject empty terminal envelopes — a titles-only plan needs a non-empty
    # draft; an `edits` proposal needs at least one target with operations
    # (delete-only / update-only batches legitimately have no hierarchy).
    if plan.status == 'proposed' and not _plan_has_content(plan):
        log_event(
            logger,
    ...
    # Staleness anchors come from the first target's roadmap context (the
    # focus roadmap for a legacy single-roadmap plan); every target keeps its
    # own copy so `is_plan_stale` can compare per roadmap.
    anchor_roadmap_id = plan.targets[0].roadmap_id if plan.targets else None
    plan.base_revision, plan.revision_token, plan.roadmap_overview_hash = _roadmap_anchor(
        session, anchor_roadmap_id
    )
    _stamp_target_anchors(session, plan)
...
def is_plan_stale(
    ...
    drifted since the plan was recorded — checked per target roadmap (a
    legacy single-roadmap plan compares the focus roadmap).
    """
    ...
    def _drifted(
        ...
        current_revision, _token, current_hash = _roadmap_anchor(session, roadmap_id)
        if recorded_revision is not None and current_revision is not None:
    ...
    if plan.targets:
        return any(
            _drifted(
                target.base_revision if target.base_revision is not None else plan.base_revision,
                target.overview_hash or plan.roadmap_overview_hash,
                target.roadmap_id,
            )
            for target in plan.targets
        )
    return _drifted(plan.base_revision, plan.roadmap_overview_hash, None)
