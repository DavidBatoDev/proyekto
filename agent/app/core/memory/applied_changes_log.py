...
def record_applied_changes_from_commit(
    ...
    *,
    ...
    ``roadmap_id`` (default: the focus roadmap) and ``run_id`` are stamped on
    the entries and the group so the prompt can group history by roadmap and
    revert can filter per roadmap. Returns the number of entries added.
    """
    ...
    roadmap_id = (
        ...
        else session.scope.focus_roadmap_id
    )
    ...
    for change in changes_raw:
        entry = _parse_change_entry(
            change, committed_at, change_id=change_id, roadmap_id=roadmap_id
        )
