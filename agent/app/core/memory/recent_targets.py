...
def append_recent_resolved_target(
    *,
    ...
    # The roadmap the node lives on (default: the focus roadmap) — batch
    # validation uses it to reject a handle/id staged against another roadmap.
    normalized_roadmap_id = (
        roadmap_id.strip()
...
def record_recent_targets_from_operations(
    *,
    ...
    """Record the nodes a staged batch touched. ``roadmap_id`` is the batch's
    roadmap (default: the focus roadmap) and is stamped on every target."""
