...
class MemoryToolHandler(ToolHandlerBase):
    async def execute(
        ...
        # Per-call: the dispatcher writes the resolved roadmap onto args
        # (the call's own roadmap_id, else the focus roadmap). Memories always
        # belong to one roadmap, so a workspace session must name it.
        ...
        if tool_name in {'save_memory', 'forget_memory'} and not roadmap_id:
            return {
                'error': {
                    'code': 'MEMORY_NEEDS_ROADMAP',
                    'message': (
                        'Memories belong to a roadmap. Pass roadmap_id (use '
                        'list_roadmaps or get_roadmap_overview to find it).'
                    ),
                }
            }
