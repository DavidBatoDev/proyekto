...
class CommentToolHandler(ToolHandlerBase):
    async def execute(
        ...
        # (the call's own roadmap_id, else the focus roadmap). Comments are
        # posted through one roadmap, so a workspace session must name it.
        ...
        if tool_name == 'add_task_comments' and not roadmap_id:
            return {
                'error': {
                    'code': 'COMMENT_NEEDS_ROADMAP',
                    'message': (
                        'Task comments are posted through a roadmap. Pass '
                        'roadmap_id (the roadmap the tasks belong to).'
                    ),
                }
            }
