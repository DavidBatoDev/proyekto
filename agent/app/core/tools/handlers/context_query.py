...
_KNOWLEDGE_MAX_PROJECT_IDS = 10

# Project-keyed reads: `project_id` (optional in a roadmap session, where the
# focus roadmap's project is the default) selects the user-scoped
# /ai/context/projects/{id} family; with no project id but a roadmap id the
# roadmap-keyed /roadmaps/{id}/ai/context/project family answers instead.
_PROJECT_KEYED_TOOL_NAMES = frozenset(
    {
        'get_project_brief',
...
class ContextQueryHandler(ToolHandlerBase):
    async def execute(
        ...
        # Per-call: the dispatcher resolved the roadmap this call targets (the
        # call's own roadmap_id, else the session's focus roadmap) and wrote it
        # back onto args. The shared session_context never carries it.
        ...
        if tool_name in _PROJECT_KEYED_TOOL_NAMES:
            return await self._execute_project_tool(
                tool_name,
                args,
                session_context,
                roadmap_id=roadmap_id,
        ...
        if tool_name == 'search_knowledge':
            ...
            sources = [
                ...
                if entry in {'chat_message', 'task_comment', 'activity_log', 'brief', 'file_chunk'}
            ] or None
            ...
            # Explicit project_ids win; otherwise the projects of the loaded
            # roadmaps (focus first); otherwise the roadmap-keyed search of the
            # call's roadmap (its linked project, resolved by the backend).
            raw_project_ids = args.get('project_ids')
            explicit_project_ids = (
                self._string_list(raw_project_ids)[:_KNOWLEDGE_MAX_PROJECT_IDS]
                if isinstance(raw_project_ids, list)
                else []
            )
            invalid_project_ids = [
                project_id for project_id in explicit_project_ids if not self._is_uuid(project_id)
            ]
            if invalid_project_ids:
                return self._invalid_argument_result(
                    arg_name='project_ids',
                    arg_value=invalid_project_ids[0],
                    message=(
                        f'project_id "{invalid_project_ids[0]}" is not a project id. Use '
                        'ids from get_workspace_overview or list_roadmaps.'
                    ),
                    error_code='INVALID_PROJECT_ID',
                )
            project_ids = explicit_project_ids
            if not project_ids:
                defaults = session_context.get('knowledge_project_ids')
                project_ids = (
                    self._string_list(defaults)[:_KNOWLEDGE_MAX_PROJECT_IDS]
                    if isinstance(defaults, list)
                    else []
                )
            if project_ids:
                coro = self._nest_client.ai_context_knowledge_search(
                    q=query[:400],
                ...
                coro = self._nest_client.context_knowledge_search(
                    roadmap_id=roadmap_id,
                    query=query[:400],
                    sources=sources,
                    limit=limit,
                ...
                result = {
                    'error': {
                        ...
                        'message': (
                            'Pass project_ids (or roadmap_id) — use get_workspace_overview '
                            'or list_roadmaps to find them.'
                        ),
                    }
                }
                log_event(
                ...
                return result
            result = await self._run_context_call(session_context, coro)
        ...
        if tool_name == 'get_roadmap_summary':
            ...
            loaded = self._notify_roadmap_loaded(session_context, roadmap_id, result)
            if isinstance(loaded, dict) and isinstance(result, dict):
                result = {
                    **result,
                    'handle_prefix': loaded.get('handle_prefix'),
        ...
        if tool_name == 'get_roadmap_overview':
            ...
            if isinstance(summary.get('error'), dict):
                ...
                loaded = self._notify_roadmap_loaded(session_context, roadmap_id, summary)
                result = {
                ...
                if isinstance(loaded, dict):
                    ...
                    result['outline'] = loaded.get('outline')
            log_event(
                self._logger,
                'tool_call_result',
                settings=self._settings,
    ...
    def _notify_roadmap_loaded(
        self,
        session_context: dict[str, Any],
        roadmap_id: str,
        summary: Any,
    ) -> dict[str, Any] | None:
        """Hand a fetched roadmap summary to the run's ``on_roadmap_loaded``
        callback (installed by the phase) so the roadmap joins the session's
        context cache with handles; returns the callback's ``{handle_prefix,
        outline}`` or ``None``."""
        callback = session_context.get('on_roadmap_loaded')
        if not callable(callback) or not roadmap_id:
            return None
        if not isinstance(summary, dict) or isinstance(summary.get('error'), dict):
            return None
        try:
            loaded = callback(roadmap_id, summary)
        except Exception as exc:  # noqa: BLE001 — registration never fails the read
            self._logger.warning(
                'on_roadmap_loaded callback failed. roadmap_id=%s error=%s', roadmap_id, exc
            )
            return None
    ...
    async def _execute_project_tool(
        ...
        *,
        ...
        project_id = (
            project_raw.strip() if isinstance(project_raw, str) and project_raw.strip() else ''
        )
        if project_id and not self._is_uuid(project_id):
            return self._invalid_argument_result(
                arg_name='project_id',
                arg_value=project_id,
                message=(
                    f'project_id "{project_id}" is not a project id. Use '
                    'get_workspace_overview or list_roadmaps to find it.'
                ),
                error_code='INVALID_PROJECT_ID',
            )
        if not project_id:
            focus_roadmap = str(session_context.get('focus_roadmap_id') or '').strip()
            focus_project = session_context.get('focus_project_id')
            # The focus project is only a valid default for the focus roadmap.
            if (
                isinstance(focus_project, str)
                and focus_project.strip()
                and (not roadmap_id or roadmap_id == focus_roadmap)
            ):
                project_id = focus_project.strip()
        if not project_id and not roadmap_id:
            result = {
                'error': {
                    'code': 'MISSING_PROJECT_ID',
                    'message': (
                        'Pass project_id — use get_workspace_overview or list_roadmaps '
                        'to find it.'
                    ),
                }
            }
        ...
        if tool_name == 'get_project_brief':
            coro = (
                self._nest_client.ai_context_project_brief(
                    project_id, auth_value, trace_id=trace_id
                )
                if project_id
                else self._nest_client.context_project_brief(
                    roadmap_id=roadmap_id, auth_header=auth_value, trace_id=trace_id
                )
            )
        elif tool_name == 'list_project_resources':
            coro = (
                self._nest_client.ai_context_project_resources(
                    project_id, auth_value, trace_id=trace_id
                )
                if project_id
                else self._nest_client.context_project_resources(
                    roadmap_id=roadmap_id, auth_header=auth_value, trace_id=trace_id
                )
            )
        elif tool_name == 'list_project_meetings':
            ...
            coro = (
                self._nest_client.ai_context_project_meetings(
                    project_id, window, limit, auth_value, trace_id=trace_id
                )
                if project_id
                else self._nest_client.context_project_meetings(
                    roadmap_id=roadmap_id,
                    window=window,
                    limit=limit,
                    auth_header=auth_value,
            ...
            coro = (
                self._nest_client.ai_context_project_member_details(
                    project_id, member_id, auth_value, trace_id=trace_id
                )
                if project_id
                else self._nest_client.context_project_member_details(
                    roadmap_id=roadmap_id,
                    member_id=member_id,
                    auth_header=auth_value,
                    trace_id=trace_id,
        ...
        else:  # pragma: no cover — guarded by _PROJECT_KEYED_TOOL_NAMES
            return {
                'error': {
                    'code': 'UNKNOWN_TOOL',
                    'message': f'Tool {tool_name} is not a project tool.',
                }
            }
