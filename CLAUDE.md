...
- agent/ - Python 3.12 FastAPI AI agent powering roadmap AI. Entry: agent/run.py -> app.main:app (port 8010). The single brain is the run orchestrator in agent/app/core/runtime/ (investigate -> propose -> execute -> verify phases, each model phase reusing the loop engine in agent/app/core/engine/, one model via the OpenAI Responses API, OPENAI_MODEL_V2). Session, run, and trace state via Upstash Redis. Deployed to Cloud Run (Docker built from repo root). See agent/CLAUDE.md.
