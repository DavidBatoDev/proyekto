...
Prodigy is a monorepo with four deployable units. Note: `SETUP.md` refers to an `api/` directory — the backend now lives in `backend/`. Treat `SETUP.md` as partially stale for paths.

- `web/` — React 19 + Vite + TanStack Router/Query/Table, MUI + Tailwind, Zustand, Lexical, XYFlow/dagre (roadmap canvas), Supabase client. Dev port 3000, path alias `@/*` → `web/src/*`.
- `backend/` — NestJS 11 API. Supabase (data/auth) + Upstash Redis (throttler storage, agent caches). Deployed to Vercel (`backend/vercel.json`, `backend/src/lambda.ts`). Feature modules under `backend/src/modules/` (auth, users, profile, projects, roadmaps, roadmap-shares, payments, admin, consultants, applications, uploads, guests, marketplace, notifications, project-time, chat).
- `agent/` — Python 3 FastAPI AI agent (LangChain/LangGraph + OpenAI) powering roadmap AI. Entry: `agent/run.py` → `app.main:app`. Orchestration lives in `agent/app/core/orchestration/` (planning, react, edits, context). Session state via Upstash Redis.
- `supabase/` — `migrations/` (source of truth for DB schema) and edge `functions/` (password reset, signup email).
- `infra/` — Terraform for Supabase storage buckets / provisioning.
- `scripts/` — Node `.mjs` benchmarks and a wrapper for agent Python tests.
...
- `npm run test:e2e` — Jest using `backend/test/jest-e2e.json`
- `npm run lint` — ESLint (auto-fix)
- `npm run check:roadmap-ai-schema` — validates the shared JSON schema in `schemas/`

### Agent (`cd agent`)
- `pip install -r requirements.txt` (venv lives at `agent/venv/`)
- `python run.py` — runs FastAPI via uvicorn, reload in dev
...
- `node scripts/validate_agent_canary_matrix.mjs` — runs `strict-canary` and `react-compat` env profiles against targeted agent unittests; non-zero exit on failure
...
5. **Web** renders the canvas with XYFlow + dagre and supports optimistic UI for epic/feature/task operations (recent commits are entirely in this area — see `feat(roadmap-optimistic-operations)`, `feat(roadmap-ai)`, `feat(planner-summary)`).
...
When changing operation shapes, update `schemas/roadmap-ai-operations.json` **and** run `npm run check:roadmap-ai-schema` from `backend/` — the schema is consumed by both NestJS validation and the Python agent's contract tests (`agent/tests/test_operation_contracts.py`).
...
- `SETUP.md` references `api/` — the directory is actually `backend/`. Supabase CLI commands still get invoked from `backend/` but `supabase/migrations/` lives at repo root.
- `web/` build runs `tsc` after `vite build`; type errors will fail the build even if Vite succeeds.
- Jest rootDir for backend is `src/` — pass paths relative to `backend/src/` (or absolute) when running a single spec.
