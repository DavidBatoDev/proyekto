-- upsert_full_roadmap wrote each row's FINAL position while stale rows
-- (deleted or reordered later in the same call) still held those positions,
-- tripping UNIQUE(roadmap_id|epic_id|feature_id, position) mid-loop:
--   duplicate key value violates unique constraint "roadmap_epics_roadmap_id_position_key"
-- Any AI commit that deleted, inserted-before, or reordered nodes could fail
-- depending on the position layout. Evacuate all live positions far out of
-- range first (same transaction), so every final position is free; stale rows
-- are removed by the trailing deletes as before.
