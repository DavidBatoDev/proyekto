-- Return the committed updated_at from upsert_full_roadmap so callers can
-- derive the revision token without an extra round trip to the DB.
-- DROP required because PostgreSQL won't allow CREATE OR REPLACE to change a return type.
