-- Extend upsert_full_roadmap to persist roadmap_milestones from p_full_state.
-- The AI commit pipeline (add_milestone / update_node / delete_node /
-- shift_dates on milestones) writes the candidate state through this RPC;
-- without this section milestone changes silently never reached the table.
-- Body is the canonical post-20260611090000 version plus the milestone
-- evacuation/upsert/delete handling (same UNIQUE(roadmap_id, position)
-- pattern as epics).
