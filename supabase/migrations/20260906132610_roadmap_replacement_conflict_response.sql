...
begin
  ...
  if not found or v_current.project_id is distinct from p_project_id then
    raise exception using errcode = 'PT409', message = 'The project roadmap changed. Refresh and try again.';
  end if;
  ...
  if v_replacement.project_id is not null then
    raise exception using errcode = 'PT409', message = 'Replacement roadmap is already linked to a project.';
  end if;
