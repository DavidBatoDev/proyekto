-- 20260611080000 rebuilt upsert_full_roadmap from a stale (April) definition,
-- resurrecting the dropped roadmap_features.status column and breaking every
-- AI commit that touches features:
--   column "status" of relation "roadmap_features" does not exist
-- Restore the canonical post-20260514120000 body and apply the intended fix:
-- evacuate live positions before the row-by-row upserts so final-position
-- writes can't collide with rows that are reordered or deleted later in the
-- same call (UNIQUE(roadmap_id|epic_id|feature_id, position)).
