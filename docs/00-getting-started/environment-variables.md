...
| Group | Vars |
| --- | --- |
| OpenAI | `OPENAI_API_KEY`, `OPENAI_MODEL_V2`, `OPENAI_V2_*` (reasoning effort, verbosity, prompt-cache mode, max output, streaming, reasoning summaries), `OPENAI_MODEL_TIMEOUT_SECONDS` |
...
| Run budgets | `AGENT_RUN_STEP_BUDGET_SECONDS`, `AGENT_RUN_HARD_DEADLINE_SECONDS`, `AGENT_RUN_MAX_STEPS`, `AGENT_RUN_LOCK_TTL_SECONDS`, `AGENT_RUN_TRANSCRIPT_TTL_SECONDS`, `AGENT_RUN_TRANSCRIPT_MAX_BYTES` |
...
| Summarizer | `AGENT_SUMMARY_MODEL`, `AGENT_SUMMARY_REASONING_EFFORT`, `AGENT_SUMMARY_TRIGGER_MESSAGES`, `_KEEP_MESSAGES`, `_MAX_CHARS` |
