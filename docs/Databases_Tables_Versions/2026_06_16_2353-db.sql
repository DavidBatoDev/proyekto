...
CREATE TABLE public.roadmap_tasks (
  ...
  updated_at timestamp with time zone DEFAULT now(),
  assignee_id uuid,
  work_type text NOT NULL DEFAULT 'real_work'::text CHECK (work_type = ANY (ARRAY['real_work'::text, 'training'::text])),
  description text,
...
CREATE TABLE public.task_time_logs (
  ...
  team_id uuid,
  work_type_snapshot text NOT NULL DEFAULT 'real_work'::text CHECK (work_type_snapshot = ANY (ARRAY['real_work'::text, 'training'::text])),
...
CREATE TABLE public.team_member_rates (
  ...
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  project_id uuid NOT NULL,
  training_hourly_rate numeric NOT NULL DEFAULT 0 CHECK (training_hourly_rate >= 0::numeric),
  weekly_limit_hours numeric,
  monthly_limit_hours numeric,
  overtime_requires_approval boolean NOT NULL DEFAULT false,
  ...
  CONSTRAINT team_member_rates_project_team_fk FOREIGN KEY (project_id) REFERENCES public.project_teams(project_id),
  CONSTRAINT team_member_rates_project_team_fk FOREIGN KEY (team_id) REFERENCES public.project_teams(team_id)
);
CREATE TABLE public.time_log_comments (
  ...
  CONSTRAINT time_log_comments_log_id_fkey FOREIGN KEY (log_id) REFERENCES public.task_time_logs(id),
  CONSTRAINT time_log_comments_author_user_id_fkey FOREIGN KEY (author_user_id) REFERENCES public.profiles(id)
);
CREATE TABLE public.invoices (
  ...
  status text NOT NULL DEFAULT 'draft'::text CHECK (status = ANY (ARRAY['draft'::text, 'issued'::text, 'sent'::text, 'paid'::text, 'void'::text])),
  currency text NOT NULL DEFAULT 'USD'::text,
  issue_date date,
  due_date date,
  notes text,
  ...
  CONSTRAINT invoices_recipient_user_id_fkey FOREIGN KEY (recipient_user_id) REFERENCES public.profiles(id),
  CONSTRAINT invoices_issuer_user_id_fkey FOREIGN KEY (issuer_user_id) REFERENCES public.profiles(id)
);
CREATE TABLE public.invoice_line_items (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  invoice_id uuid NOT NULL,
  source_type text NOT NULL DEFAULT 'manual'::text CHECK (source_type = ANY (ARRAY['manual'::text, 'time_log'::text])),
  source_log_id uuid,
  ...
  CONSTRAINT invoice_line_items_source_log_id_fkey FOREIGN KEY (source_log_id) REFERENCES public.task_time_logs(id)
);
CREATE TABLE public.invoice_documents (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  invoice_id uuid NOT NULL,
  kind text NOT NULL DEFAULT 'pdf'::text CHECK (kind = 'pdf'::text),
  storage_path text NOT NULL,
  created_by uuid,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  ...
  CONSTRAINT invoice_documents_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.profiles(id)
);
CREATE TABLE public.task_activity_log (
  ...
  CONSTRAINT task_activity_log_task_id_fkey FOREIGN KEY (task_id) REFERENCES public.roadmap_tasks(id),
  CONSTRAINT task_activity_log_changed_by_fkey FOREIGN KEY (changed_by) REFERENCES public.profiles(id)
);
CREATE TABLE public.task_dependencies (
  ...
  CONSTRAINT task_dependencies_blocking_task_id_fkey FOREIGN KEY (blocking_task_id) REFERENCES public.roadmap_tasks(id),
  CONSTRAINT task_dependencies_blocked_task_id_fkey FOREIGN KEY (blocked_task_id) REFERENCES public.roadmap_tasks(id),
  CONSTRAINT task_dependencies_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.profiles(id)
);
CREATE TABLE public.email_verification_codes (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  email text NOT NULL,
  purpose text NOT NULL CHECK (purpose = ANY (ARRAY['signup'::text, 'login'::text])),
  code_hash text NOT NULL,
  salt text NOT NULL,
  expires_at timestamp with time zone NOT NULL DEFAULT (now() + '00:10:00'::interval),
  consumed_at timestamp with time zone,
...
CREATE TABLE public.roadmap_ai_memories (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  roadmap_id uuid NOT NULL,
  content text NOT NULL CHECK (char_length(content) >= 1 AND char_length(content) <= 500),
  source text NOT NULL DEFAULT 'user_request'::text CHECK (source = ANY (ARRAY['user_request'::text, 'inferred'::text])),
  created_by uuid,
  is_active boolean NOT NULL DEFAULT true,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT roadmap_ai_memories_pkey PRIMARY KEY (id),
  CONSTRAINT roadmap_ai_memories_roadmap_id_fkey FOREIGN KEY (roadmap_id) REFERENCES public.roadmaps(id),
  CONSTRAINT roadmap_ai_memories_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id)
);