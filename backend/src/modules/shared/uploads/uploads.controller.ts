...
export class UploadsService {
  ...
  /**
   * Writes server-generated bytes straight to the private R2 bucket.
   *
   * Distinct from `uploadFile`, which validates a browser upload against
   * BUCKET_CONFIG. Rendered documents (invoice PDFs, signed agreements) never
   * come from a client, so there is no untrusted mimetype or size to police —
   * but they are private, hence the private bucket and presigned reads.
   */
