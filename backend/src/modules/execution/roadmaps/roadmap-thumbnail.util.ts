/**
 * Deterministic default roadmap thumbnails (backend port).
 *
 * Produces an SVG data URI (gradient background + up-to-2-char initials) from a
 * stable seed (roadmap id, or the name when no id exists yet). Mirrors the web
 * generator in web/src/lib/roadmapThumbnail.ts and the backfill migration
 * (supabase/migrations/20260701120000_backfill_and_require_roadmap_preview_url.sql)
 * so server-generated defaults match the rest of the app. Dependency-free so all
 * sides stay in sync.
 */

// Gradient palette [from, to]. Kept in sync with the web util + backfill SQL.
...
/** Up-to-2-char uppercase initials from a name. */
