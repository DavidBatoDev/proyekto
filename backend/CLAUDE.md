...
- Guards: SupabaseAuthGuard (default auth), ConsultantOnlyGuard (active-consultant surfaces, requiring `profiles.role='consultant'` plus `is_consultant_verified`). Every new route needs a guard unless it is deliberately public.
